...
contains 
  ...
  subroutine read_size(fileName, ncol, nlay, nexp) 
    ...
    integer,         optional, intent(  out) :: ncol, nlay,  
    ! ---------------------------
    integer :: ncid
    ! ---------------------------
  ...
  subroutine read_and_block_lw_bc(fileName, blocksize, 
    ...
    integer :: nblocks 
    ! ---------------------------
    if(any([ncol_l, nlay_l, nexp_l]  == 0)) call stop_on_err("read_and_block_lw_bc: Haven't read problem size yet.") 
    if(mod(ncol_l*nexp_l, blocksize) /= 0 ) call stop_on_err("read_and_block_lw_bc: number of columns doesn't fit evenly into blocks.") 
    nblocks = (ncol_l*nexp_l)/blocksize
    !
    ! Check that output arrays are sized correctly : blocksize, nlay, (ncol * nexp)/blocksize
    !
  ...
  subroutine read_and_block_gases_ty(fileName, blocksize, gas_names, gas_conc_array)
    character(len=*),           intent(in   ) :: fileName
    integer,                    intent(in   ) :: blocksize, b, g
    ...
    gas_conc_temp_3d = reshape(read_field(ncid, "water_vapor", ncol_l, nlay_l, nexp_l), & 
                               shape = [blocksize, nblocks, nlay_l], order = [1, 3, 2]) 
    do b = 1, nblocks 
      call stop_on_err(gas_conc_array(b)%set_vmr('h2o', gas_conc_temp[:,:,b])) 
    end do 
    gas_conc_temp_3d = reshape(read_field(ncid, "ozone", ncol_l, nlay_l, nexp_l), & 
                               shape = [blocksize, nblocks, nlay_l], order = [1, 3, 2]) 
    do b = 1, nblocks 
      call stop_on_err(gas_conc_array(b)%set_vmr('o3', gas_conc_temp[:,:,b])) 
    end do 
    ...
    do g = 1, size(gas_names) 
      ...
      if(any(gas_name_in_file == ['h2o', 'o3'])) cycle 
      !
